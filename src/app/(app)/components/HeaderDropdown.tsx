    ...
    <div className="relative" ref={dropdownRef}>
      ...
      {isOpen && (
        <div className="absolute right-0 mt-2 w-56 bg-white dark:bg-darkbg-secondary border border-slate-200 dark:border-slate-800 rounded-xl shadow-xl z-50 py-1 animate-in fade-in slide-in-from-top-2">
          ...
          <button 
            onClick={() => { setIsOpen(false); router.push('/settings?tab=profile'); }} 
          ...
          >
            <CircleUserRound className="w-4 h-4 text-slate-400" /> プロフィール編集
          </button>
<button 
          ...
          <button
            ...
            disabled={isLocked}
            title={isLocked ? 'タイマー実行中・問題に取り組み中は他の画面へ移動できません' : undefined}
            className="w-full text-left px-4 py-2 text-sm text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 flex items-center gap-2 disabled:opacity-40 disabled:cursor-not-allowed"
          >
