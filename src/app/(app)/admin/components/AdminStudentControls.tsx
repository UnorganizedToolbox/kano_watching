...
import { Lock, Unlock, ShieldOff, ShieldCheck, Trash2, GraduationCap, Building2 } from 'lucide-react';
import { setStudentNickname, setStudentStatus, deleteStudent, promoteToTeacher, approveMember, setMemberOrganization } from '../actions';
...
export default function AdminStudentControls({
  ...
  const [memberOrgId, setMemberOrgId] = useState(initialOrganizationId || '');
  ...
  const handlePromote = () => {
    ...
    startTransition(async () => {
      try {
        await promoteToTeacher(studentId, isAdmin ? (promoteOrgId || null) : undefined);
        setRole('teacher');
  ...
  const handleChangeOrganization = () => {
    startTransition(async () => {
      try {
        await setMemberOrganization(studentId, memberOrgId || null);
        setMessage('所属団体を変更しました。');
        setTimeout(() => setMessage(''), 3000);
      } catch (e) {
    ...
    <div className="card-glass bg-white dark:bg-darkbg-secondary border border-slate-200 dark:border-slate-800 rounded-2xl p-6 shadow-sm">
      ...
      <div className="mb-4 pb-4 border-b border-slate-100 dark:border-slate-800 space-y-1.5">
        ...
        <div className="flex justify-between items-center text-sm">
          ...
          <span className="font-bold text-slate-700 dark:text-slate-300">{role === 'teacher' ? '教師' : '生徒'}</span>
        </div>
        <div className="flex justify-between items-center text-sm">
          <span className="text-slate-500 dark:text-slate-400">所属団体</span>
          <span className="font-bold text-slate-700 dark:text-slate-300">{orgName(initialOrganizationId) || '個人'}</span>
        </div>
        <div className="flex justify-between items-center text-sm">
      ...
      <div className="pt-4 border-t border-slate-100 dark:border-slate-800">
        ...
        {status === 'pending' ? (
          ...
          <p className="text-xs text-slate-400">対応中のアカウントです。</p>
        )}
      </div>
      ...
      {role === 'student' && (
        <div className="pt-4 mt-4 border-t border-slate-100 dark:border-slate-800">
          ...
          {isAdmin && (
            <select
              value={promoteOrgId}
              onChange={(e) => setPromoteOrgId(e.target.value)}
              className="w-full mb-2 px-3 py-2 text-sm border border-slate-200 dark:border-slate-700 rounded-lg bg-slate-50 dark:bg-slate-900 text-slate-900 dark:text-white outline-none"
            >
          ...
          <button
            ...
            disabled={isPending}
            className="w-full py-2 rounded-lg text-xs font-bold flex items-center justify-center gap-2 disabled:opacity-50 bg-indigo-50 text-indigo-700 dark:bg-indigo-900/20 dark:text-indigo-400"
          >
      ...
      {isAdmin && (
        <div className="pt-4 mt-4 border-t border-slate-100 dark:border-slate-800">
          <label className="text-xs font-bold text-slate-500 block mb-2">所属団体の変更</label>
          <div className="flex gap-2">
            ...
            <button
              ...
              disabled={isPending}
              className="px-4 py-2 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 rounded-lg text-xs font-bold disabled:opacity-50 flex items-center gap-1 shrink-0"
            >
