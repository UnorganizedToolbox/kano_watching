...
function triggerBeepSequence(): void {
  if (!globalAlertEl) return;
  ...
  const handleEnd = () => {
    ...
    if (loopCount < 3) {
      setTimeout(() => {
        if (globalAlertEl) {
          globalAlertEl.play().catch(e => console.warn("Beep replay blocked:", e));
        }
      }, 120);
    } else {
      if (globalAlertEl) {
        globalAlertEl.removeEventListener('ended', handleEnd);
      }
    }
  };
  
  globalAlertEl.addEventListener('ended', handleEnd);
  globalAlertEl.play().catch(e => {
    console.warn("Alert play failed. Retrying in foreground:", e);
  });
}
